...
- **Credentials.** Credentials are re-resolved per operation, so no action is needed unless a reference name changed — in that case update `userNameRef` / `passwordRef` / `appSecretRef` in the settings. `userNameRef` is now required in `app` mode too (it names the 集成用户).
