 ...
 */
export function assertSafePublicUrl(rawUrl) {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
        throw new KdError('kd/invalid-config', `Forbidden protocol "${parsed.protocol}". Only http: and https: are allowed.`);
    }
    ...
    if (isPrivateOrLocalHost(hostname)) {
        throw new KdError('kd/invalid-config', `SSRF protection: host "${hostname}" is localhost, loopback, private, or reserved address.`);
    }
    return parsed;
}
/**
...
export function isPrivateOrLocalHost(rawHostname) {
    ...
    if (host.includes(':')) {
        ...
        if (host.endsWith('::1') && /^0*(::1|:0*:1)$/.test(host))
            return true;
    }
    return false;
}
