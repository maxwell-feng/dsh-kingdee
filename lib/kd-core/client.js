 ...
 */
import { buildAppSecretLoginPayload, buildLoginPayload, businessHeaders, validateConfig } from "./auth.js";
import { assertSuccess, KdError, toKdError } from "./errors.js";
import { extractKdsvcCookie, joinUrl, parseEnvelope, parseLoginOutcome } from "./envelope.js";
...
export class KdClient {
    ...
    async logout() {
        ...
        const body = { acctID: this.cfg.acctId };
        if (this.cfg.userName)
            body.username = this.cfg.userName;
        if (this.cfg.password)
            body.password = this.cfg.password;
        try {
    ...
    async queryBusinessData(params) {
        return this.post(this.form('QueryBusinessData'), queryBody(params));
    }
    /** Save a form (create or update a bill / base record). */
    async save(params) {
        const body = { FormId: params.formId, Data: params.data };
        if (params.interaction !== undefined)
            body.Interaction = params.interaction;
        if (params.isAutoSubmitAndAudit !== undefined)
            body.IsAutoSubmitAndAudit = params.isAutoSubmitAndAudit;
        return this.post(this.form('Save'), body);
    }
    /** Batch-save multiple records in one call. */
    async batchSave(params) {
        const body = { FormId: params.formId, Data: params.records };
        if (params.interaction !== undefined)
            body.Interaction = params.interaction;
        if (params.isAutoSubmitAndAudit !== undefined)
            body.IsAutoSubmitAndAudit = params.isAutoSubmitAndAudit;
    ...
    /** Submit a form. */
    async submit(params) {
        const body = { FormId: params.formId };
        if (params.ids && params.ids.length > 0)
            body.Ids = params.ids.join(',');
        if (params.numbers && params.numbers.length > 0)
            body.Numbers = params.numbers.join(',');
...
function queryBody(params) {
    ...
    if (params.filter !== undefined)
        body.FilterString = params.filter;
    if (params.orderString !== undefined)
        body.OrderString = params.orderString;
    ...
    if (params.startRowIndex !== undefined)
        body.StartRowIndex = params.startRowIndex;
