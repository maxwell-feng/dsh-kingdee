...
- **One Chinese entry covered a release at less depth than English.** The 0.3.0 entry carried a single line against the English entry's seam detail, dependency bump and documentation refresh; it now matches. The Chinese documentation list also links CHANGELOG.zh.md instead of the English changelog.
...
- **Configuration migrated to the 0.1.7 volatile schema**: every editable field is declared `.volatile()`, so `apply` receives one live reference per field. The plugin no longer registers a settings section — `ctx.settings.installSection`, `ctx.inject(['settings'])`, and the `@deepseek-ai/dsh-settings` dependency are gone. The Host discovers the exported `Config` schema itself (`entry.fiber.runtime.Config`) and renders this entry's form, keyed by the profile row id `kingdee`.
...
- **Client card contract documented**: `src/client/settings-card.ts` is described as the browser half of the same per-entry `ConfigForm` the Plugins page owns.

### Fixed

- **The client half's `dsh.client.inject` list was incomplete.** The card registers into `plugins.row.config` / `plugins.bundle.config` (owned by `@deepseek-ai/dsh-client-ui-plugin-manager`) and reads `ctx.remote` and `ctx.connection`, but only `@deepseek-ai/dsh-client-locale` and `@deepseek-ai/dsh-client-ui-settings` were declared. It now declares `@deepseek-ai/dsh-client-ui-plugin-manager`, `@deepseek-ai/dsh-api-remotes`, and `@deepseek-ai/dsh-client-connection` too, matching the official client cards that use the same services.
...
- `pnpm install --frozen-lockfile` passes pnpm's supply-chain gate; the declared DSH peers pass DeepSeek Harness's own `evaluatePluginCompatibility` against runtime `0.1.7-rc.1`.
