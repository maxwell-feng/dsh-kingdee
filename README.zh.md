...
> 已在 deepseek-harness **0.1.7-rc.1** 上验证：`pnpm run typecheck` 零错误、**15** 项单元测试通过（`pnpm test`），且 bundle 补丁在真实 `0.1.7-rc.1` profile 中作为 `# == dsh-kingdee` 层正常生效（`dsh plugin --profile <name> add` → `dsh --profile <name> --dump-config`）。**未进行真实账套联调验证。**
...
- **未做真实账套联调。** 本文档的所有内容均以类型检查、单元测试套件与真实 `0.1.7-rc.1` profile 安装为依据，而非在运行中的金蝶账套上验证。
