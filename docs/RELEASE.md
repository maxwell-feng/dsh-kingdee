...
Ninth release of **dsh-kingdee**, the Kingdee Cloud Starry Sky secondary-development plugin for DeepSeek Harness. Fully adapted to **Kingdee Cloud Starry Sky V9.0 Enterprise Edition** (金蝶云·星空 V9.0 企业版, as well as V8.x / V9.1), hardened with enterprise-grade **SSRF security defenses**, and cleaned of legacy dead code. Verified on **deepseek-harness `0.1.5-rc.2`**.
...
- **Update**: `dsh plugin update dsh-kingdee` (or `dsh plugin add dsh-kingdee@0.6.1`).
...
- **Usage**: Configure connection in the **Plugins → kingdee** settings card, set credential environment variables (`DSH_KINGDEE_USER` / `DSH_KINGDEE_PASSWORD` / `DSH_KINGDEE_APP_SECRET`), then invoke `kingdee_*` tools in chat sessions. See [USAGE.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/USAGE.md) / [USAGE.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/USAGE.zh.md) for the full tool reference.
  **使用**：在 **Plugins → kingdee** 设置卡片配置连接，配置凭据环境变量，然后在会话中调用 `kingdee_*` 工具。完整工具说明见 [USAGE.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/USAGE.zh.md)。
- **Config**: See [CONFIG.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CONFIG.md) / [CONFIG.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CONFIG.zh.md) for full configuration reference.
  **配置**：详细配置见 [CONFIG.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CONFIG.zh.md)。支持 `baseUrl`、`acctId`、`authMode`（`user` | `app`）、`appId`、凭据引用、`organization`、`timeoutMs`、`mock`、可选 `serviceEndpoints`。
...
- **Dead Code Cleanup & Pruning** — Removed unused legacy interfaces (`KdToolResult`), redundant helper functions (`parseEnvelopeFromText`), and dead error codes (`kd/not-found`), optimizing bundle size and type cleanliness.
...
- **Robust Verification** — Native test suites passing, 0 TypeScript compilation errors.
...
- **死代码清理与极致精简** —— 彻底移除废弃的 `KdToolResult`、`parseEnvelopeFromText` 及死错误码 `'kd/not-found'`，精简包体积并提升类型纯净度。
...
See [INSTALL.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/INSTALL.md) / [INSTALL.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/INSTALL.zh.md). Upgrade and removal are in [UPDATE.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/UPDATE.md) and [UNINSTALL.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/UNINSTALL.md).

## Links / 相关链接

- Homepage / 主页: https://github.com/maxwell-feng/dsh-kingdee
- Usage Guide / 使用文档: [USAGE.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/USAGE.md) | [USAGE.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/USAGE.zh.md)
- Config Guide / 配置文档: [CONFIG.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CONFIG.md) | [CONFIG.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CONFIG.zh.md)
- Changelog / 变更日志: [CHANGELOG.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CHANGELOG.md) | [CHANGELOG.zh.md](https://github.com/maxwell-feng/dsh-kingdee/blob/v0.6.1/CHANGELOG.zh.md)
